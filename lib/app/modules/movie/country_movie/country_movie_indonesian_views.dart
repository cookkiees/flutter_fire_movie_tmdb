...
class CountryMovieIndonesianViews extends GetView<CountryMovieController> {
  const CountryMovieIndonesianViews({
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        return Obx(
          () => controller.isLoadingKoreanMovie.value
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: GridView.builder(
                            itemCount: controller.koreanMovie.length,
                            shrinkWrap: true,
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              var movie = controller.indonesiaMovie[index];

                              return Padding(
                                padding: const EdgeInsets.only(right: 16),
                                child: CustomMovieCardWidget(
                                  onTap: () {
                                    if (controller.indonesiaMovie.isNotEmpty) {
                                      Get.toNamed(
                                        '${AppRoutes.details}/${controller.indonesiaMovie[index].id}',
                                      );
                                    }
                                    details.refreshData();
                                  },
