...
class GenreFantasyMovieViews extends GetView<GenreMovieController> {
  const GenreFantasyMovieViews({
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        return Obx(
          () => controller.isLoadingFantasyMovie.value
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: GridView.builder(
                            itemCount: controller.fantasyMovie.length,
                            shrinkWrap: true,
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              var movie = controller.fantasyMovie[index];

                              return Padding(
                                padding: const EdgeInsets.only(right: 16),
                                child: CustomMovieCardWidget(
                                  onTap: () {
                                    if (controller.fantasyMovie.isNotEmpty) {
                                      Get.toNamed(
                                        '${AppRoutes.details}/${controller.fantasyMovie[index].id}',
                                      );
                                    }
                                    details.refreshData();
                                  },
