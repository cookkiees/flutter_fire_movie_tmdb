...
class GenreActionMovieViews extends GetView<GenreMovieController> {
  const GenreActionMovieViews({
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        return Obx(
          () => controller.isLoadingActionMovie.value
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: GridView.builder(
                            itemCount: controller.actionMovie.length,
                            shrinkWrap: true,
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              var movie = controller.actionMovie[index];

                              return Padding(
                                padding: const EdgeInsets.only(right: 16),
                                child: CustomMovieCardWidget(
                                  onTap: () {
                                    if (controller.actionMovie.isNotEmpty) {
                                      Get.toNamed(
                                        '${AppRoutes.details}/${controller.actionMovie[index].id}',
                                      );
                                    }
                                    details.refreshData();
                                  },
